 ...
 */
/**
 * One element on the canvas, by container and position.
 *
 * Both, because a position alone stopped identifying anything once panels became
 * containers: `[data-element-index="0"]` is the first element of the page *and* the
 * first element of every panel on it.
 */
function elementAt(page: Page, index: number, container = 'p1'): Locator {
  return canvasOf(page).locator(`[data-in-container="${container}"][data-element-index="${String(index)}"]`);
}
...
test('parity/K2-drag: an item from the toolbox lands where it was dropped', async ({ page }) => {
  ...
  const order = await orderIn(page);
...
test('parity/K2-drag: an element is moved by its handle', async ({ page }) => {
  ...
  // as in a row.
  expect(await orderIn(page)).toEqual(['draftTier', 'draftScore', 'draftName', 'draftGroup', 'draftEmpty']);
});

test('parity/K2-drag: a two-column canvas is aimed left and right', async ({ page }) => {
  const surface = surfaceOf(page);
  const target = elementAt(page, 1);
  ...
  expect(await orderIn(page)).toEqual(['draftTier', 'draftName', 'draftScore', 'draftGroup', 'draftEmpty']);
});
...
test('parity/K2-nesting: a question is dragged into a panel', async ({ page }) => {
  const surface = surfaceOf(page);
  const start = await pointIn(surface.getByRole('button', { name: 'Move draftName' }));
  await page.mouse.move(start.x, start.y);
  await page.mouse.down();
  ...
  // Onto the question already inside the panel — a slot the canvas could not offer at
  // all until it adorned what a panel renderer had drawn.
  const inside = await pointIn(elementAt(page, 0, 'draftGroup'), 0.9);
  await page.mouse.move(inside.x, inside.y, { steps: 8 });
  await page.mouse.up();

  expect(await orderIn(page, 'draftGroup')).toEqual(['draftRole', 'draftName']);
  expect(await orderIn(page)).toEqual(['draftTier', 'draftScore', 'draftGroup', 'draftEmpty']);
});

test('parity/K2-nesting: an empty panel is a place a drop can land', async ({ page }) => {
  const surface = surfaceOf(page);
  const start = await pointIn(surface.getByRole('button', { name: 'Move draftName' }));
  await page.mouse.move(start.x, start.y);
  await page.mouse.down();
  ...
  // It has no child to aim at, so it says so itself — otherwise a panel a designer has
  // just added is the one place on the canvas nothing can go.
  const empty = await pointIn(canvasOf(page).locator('[data-empty-container="draftEmpty"]'));
  await page.mouse.move(empty.x, empty.y, { steps: 8 });
  await page.mouse.up();

  expect(await orderIn(page, 'draftEmpty')).toEqual(['draftName']);
});

test('parity/K2-nesting: a question comes back out of a panel', async ({ page }) => {
  const surface = surfaceOf(page);
  const start = await pointIn(surface.getByRole('button', { name: 'Move draftRole' }));
  await page.mouse.move(start.x, start.y);
  await page.mouse.down();
  await page.mouse.move(start.x + 4, start.y + 4);
  const out = await pointIn(elementAt(page, 0), 0.1);
  await page.mouse.move(out.x, out.y, { steps: 8 });
  await page.mouse.up();
  ...
  expect(await orderIn(page, 'draftGroup')).toEqual([]);
});
