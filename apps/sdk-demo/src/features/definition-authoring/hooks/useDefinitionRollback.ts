...
export interface DefinitionRollbackState {
  ...
  readonly activate: (releaseDigest: string, expectedVersion: number) => Promise<void>;
}
...
export function useDefinitionRollback(
  ...
  const activate = useCallback(async (
    ...
    setError(undefined);
    try {
      await client.activate(managedName, environmentName, releaseDigest, expectedVersion);
      setNeedsLogin(false);
    } catch (reason) {
      setNeedsLogin(reason instanceof DefinitionAuthoringError && reason.status === 401);
      setError(reason instanceof Error ? reason.message : 'Definition activation failed.');
    } finally {
      setWorking(false);
      refresh();
    }
  }, [client, environmentName, managedName, refresh]);
  return { error, isWorking, needsLogin, activate };
}
