...
| Maintained Creator-adapter requirements | `PageElementDecoratorProvider`, `PageElementSlot`, `PageElementSlotDecoratorProvider`, `TextRendererProvider`, `QuestionErrors`, `QuestionTitleContent`, `questionErrorId`, `questionId`, `useIdScope`, `QuestionRenderersProvider`, `readOnlyAction`, `reorderAnnouncement` | Creator adorners, nested container rendering, property actions, and placement narration import these through the public package seam. The two decorators wrap different things and both are needed: `PageElementDecoratorProvider` wraps an element *inside* its layout slot, which is where an adorner belongs, and `PageElementSlotDecoratorProvider` wraps the slot itself, which is the only way to add a node a container lays out as one of its own children — the drop placeholder (P12) has to be a cell of the container's grid or it cannot say which of two side-by-side positions a drop means. |
